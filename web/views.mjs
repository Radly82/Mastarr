...
export function settings(state) {
  ...
  return `<div class="page-heading"><div><span class="eyebrow">MAKE YOURSELF AT HOME</span><h1>A setup that stays yours<span class="accent">.</span></h1><p>Connect once. Your server takes care of every screen.</p></div>${badge('online', 'Server-side settings')}</div><div class="notice">${icon('shield')}<div><strong>Your credentials never leave the server after saving.</strong><span>Leave an API key blank to keep the existing value. No browser storage. No keys in shareable URLs.</span></div></div>${!state.session.secure ? `<div class="notice warning">${icon('warning')}<div><strong>This connection uses HTTP.</strong><span>Use only on a trusted LAN or VPN. For remote access, configure an HTTPS reverse proxy, PUBLIC_ORIGIN, and SECURE_COOKIES=true.</span></div></div>` : ''}
