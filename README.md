...
**Your media, together.** A private, self-hosted control room for movies, series, downloads and release calendars. Version 2.0.4.
...
`mastarr.xml` defines the appdata mount, port 8686 and an unprivileged runtime. Its versioned Docker Hub image is produced by the **Release Docker** workflow. Until that workflow has successfully published the image, build locally and use `mastarr:2.0.4` as the repository/image instead. The Docker Hub repository must be public if anonymous pulls are desired.
...
docker build -t mastarr:2.0.4 .
node scripts/docker-smoke.mjs mastarr:2.0.4
```
...
```sh
docker run --rm -p 127.0.0.1:8765:8686 -e DEMO_MODE=true mastarr:2.0.4
```
