...
`mastarr.xml` defines the appdata mount, port 8686 and an unprivileged runtime. Its versioned GHCR image is produced by the **Release Docker** workflow. Until that workflow has successfully published the image, build locally and use `mastarr:14.0.0` as the repository/image instead. For a newly published GHCR package, the owner must also make the package public if anonymous pulls are desired.
...
docker build -t mastarr:14.0.0 .
node scripts/docker-smoke.mjs mastarr:14.0.0
```
...
```sh
docker run --rm -p 127.0.0.1:8765:8686 -e DEMO_MODE=true mastarr:14.0.0
```
